...
日本語の会話，一般向け文章，技術文書，レポート，学位論文，および論文の作成，修正，校正，
レビューに用いるCodex skillである．
文章の形式を表す文章レベルと，変更範囲を表す作業モードを別々に選択する．

## 構成

- `SKILL.md`には，共通規則，文章レベルとモードの選択基準，およびtextlintの実行条件がある．
- `references/`には，会話調・一般向け文章，正式な文書，校正，および対訳辞書がある．
...
共通規則はすべての日本語出力へ適用する．
その上で，文章の用途と求められる形式に応じて，次のいずれかの文章レベルを選ぶ．

| 文章レベル | 対象 | 基本の文体 | textlint |
|---|---|---|---|
| `conversation` | 会話，通常の説明，一般向けの記事，ブログ，簡単なノート | です・ます調 | 内容に応じて`conversation`を使う |
| `document` | レポート，技術文書，仕様書，論文，学位論文，投稿原稿 | だ・である調 | `document`を使う |

保存されるかどうかだけでは文章レベルを決めない．
ブログなどの保存される文章でも，一般向けまたは会話調であれば`conversation`を選べる．
`document`には学術文書向けの厳密さも統合し，引用，数式，図，およびプロットに固有の規則は，
対象が存在する場合だけ適用する．
作業モードは，文章レベルとは独立して選ぶ．
...
`conversation`の文章レベルに使う．
複数の前提や根拠を扱う説明，数式の導出，論証，因果関係，手順，複数案の比較，または複数段落の
一般向け文章を検査する．
